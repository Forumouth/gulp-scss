(function() {
  var compile, gutil, inject, mapConverter, merge, path, q, testing, through;
    ...
    "exec", "mkdirp", "fs", function(exec, mkdirp, fs) {
      return function(opts) {
        return function(file, enc, cb) {
          ...
          q.nfcall(mkdirp, options.tmpPath).then(function() {
            var defer, tmp, tmpWriteStream;
            defer = q.defer();
            tmp = file.clone();
            tmpWriteStream = fs.createWriteStream(path.join(options.tmpPath, tmp.relative));
            ...
            command.push("scss");
            command = command.concat("--sourcemap=" + options.sourcemap, path.join(options.tmpPath, file.relative), path.join(options.tmpPath, gutil.replaceExtension(file.relative, ".css")));
            ...
            file.path = path.join(options.tmpPath, gutil.replaceExtension(file.relative, ".css"));
            contents = fs.readFileSync(file.path).toString("utf-8");
            sourcemap = mapConverter.fromMapFileSource(contents, options.tmpPath);

      ...
      return function(opts) {
        return function(file, enc, cb) {
          ...
          q.nfcall(mkdirp, tmpDir).then(function() {
            ...
            defer = q.defer();
            tmp = file.clone();
            tmpWriteStream = fs.createWriteStream(path.join(tmpDir, tmp.relative));
            ...
            command.push("scss");
            command = command.concat(["--sourcemap=" + options.sourcemap, path.join(tmpDir, file.relative), path.join(tmpDir, gutil.replaceExtension(file.relative, ".css"))]);
            ...
            contents = fs.readFileSync(path.join(tmpDir, gutil.replaceExtension(file.relative, ".css"))).toString("utf-8");
